...
def PolygonPatchCollection(polygons, **kwargs):
    list_of_polygons = []
    for p in polygons.geoms:
        _poly = PathPatch(PolygonPath(p), **kwargs)
        list_of_polygons.append(_poly)

    return PatchCollection(list_of_polygons, **kwargs)

"""
def PolygonPatch3d(polygons, **kwargs):
    list_of_polygons = []
    for p in polygons.geoms:
        _poly = PathPatch(PolygonPath(p), **kwargs)
        _poly3d =
